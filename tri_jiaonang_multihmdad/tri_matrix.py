...
def comtriple(X,W,L_parameter,D_parameter,U_parameter,V_parameter,k,m,bool):
    ...
    for i in range(1000):
        ...
        U_down=2*e_parameter*np.dot(np.dot(np.multiply(W,np.dot(np.dot(U,S),np.transpose(V))),V),np.transpose(S))+D_parameter*np.dot(D2,U)+2*U_parameter*U
        ...
        V_down=2*e_parameter*np.dot(np.dot(np.multiply(np.transpose(W),np.dot(np.dot(V,np.transpose(S)),np.transpose(U))),U),S)+L_parameter*np.dot(D1,V)+2*V_parameter*V
...
def colum_triple(interaction,L_parameter,D_parameter,U_parameter,V_parameter,k,m,bool):
    ...
    for i in range(5):
        ...
        tmpX=np.hstack((X,np.zeros(((nd,20)))))
    ...
    return np.sum(auc) / 5, np.sum(aup) / 5, np.sum(precition) / 5, np.sum(recall) / 5, np.sum(fmeasure) / 5, np.sum(
        auc0) / 5, np.sum(aup0) / 5, np.sum(precition0) / 5, np.sum(recall0) / 5, np.sum(fmeasure0) / 5, np.sum(
        auc1) / 5, \
           np.sum(aup1) / 5, np.sum(precition1) / 5, np.sum(recall1) / 5, np.sum(fmeasure1) / 5,np.sum(auc2) / 5, np.sum(aup2) / 5, np.sum(precition2) / 5, np.sum(recall2) / 5, np.sum(fmeasure2) / 5,np.sum(auc3) / 5, np.sum(aup3) / 5, np.sum(precition3) / 5, np.sum(recall3) / 5, np.sum(fmeasure3) / 5, np.sum(
        auc4) / 5, np.sum(aup4) / 5, np.sum(precition4) / 5, np.sum(recall4) / 5, np.sum(fmeasure4) / 5,np.sum(auc5) / 5, np.sum(aup5) / 5, np.sum(precition5) / 5, np.sum(recall5) / 5, np.sum(fmeasure5) / 5
...
def random1_triple(interaction,L_parameter,D_parameter,U_parameter,V_parameter,k,m,bool):
    nd = interaction.shape[0]
    nm = interaction.shape[1]
    f1=open('data/knowndiseasemicrobeinteraction.txt','r')
    txt=f1.read()
    txt = txt.split('\n')
    f1.close()
    auc = np.zeros((5, 1))
...
def case(interaction,L_parameter,D_parameter,U_parameter,V_parameter,S_parameter,S1_parameter,k,m,bool):
    nd = interaction.shape[0]
    nm = interaction.shape[1]
    onesindex = []
    for i in range(nd):
    ...
    for cv in range(1):
        ...
        captrain1x2 = []
        captrain1y = []
        F, R, kd, km = comtriple(X, W, L_parameter, D_parameter, U_parameter, V_parameter, S_parameter, S1_parameter, k,
                                 m)
        numtest = 0
        tmpF = np.hstack((F, np.zeros((nd, 20))))
        tmpR = np.hstack((R, np.zeros((nd, 20))))
...
if __name__ == "__main__":
    ...
    print('MVFA-NMTF-CAPNN' + str(BOOL) + ',auc=' + str(np.sum(auc4) / n) + ',varauc=' + str(np.var(auc4)) + ',aup=' + str(
    ...
    print('MVFA-BI-CAPNN' + str(BOOL) + ',auc=' + str(np.sum(auc5) / n) + ',varauc=' + str(np.var(auc5)) + ',aup=' + str(
